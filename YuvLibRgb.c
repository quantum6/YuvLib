...
static int YUV_RgbxToUyvy(unsigned char* pRgb,
                          unsigned char* pUyvy,
                          int width,
                          int height,
                          int nPixelSize)
{
    ...
    unsigned char* pUyvyData = (unsigned char *)pUyvy;
    ...
    for (int i = 0; i<height; ++i)
    {
        ...
        {
            B1 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2    );
            G1 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2 + 1);
            R1 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2 + 2);

            B2 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2 + nPixelSize    );
            G2 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2 + nPixelSize + 1);
            R2 = *(pRgbData + i*width * nPixelSize + j*nPixelSize*2 + nPixelSize + 2);
            ...
            *(pUyvyData + i*width * 2 + j * 4    ) = U1;
            *(pUyvyData + i*width * 2 + j * 4 + 1) = Y1;
            *(pUyvyData + i*width * 2 + j * 4 + 2) = V1;
            *(pUyvyData + i*width * 2 + j * 4 + 3) = Y2;
...
int YUV_RgbToUyvy(unsigned char* pRgb,
                          ...
                          int height)
{
    return YUV_RgbxToUyvy(pRgb, pUyvy, width, height, 3);
}
...
int YUV_RgbaToUyvy(unsigned char* pRgb,
                          ...
                          int height)
{
    return YUV_RgbxToUyvy(pRgb, pUyvy, width, height, 4);
}

static void YUV_Rgb_FillLine(char* pLine,
                             int start,
                             int width,
                             int pixel,
                             int block)
{
    int hCount = (width +block-1)/block;
    int colors[] = { 0xFFFFFF, 0xFF0000, 0x00FF00, 0x0000FF, 
                     0xFFFF00, 0xFF00FF, 0x00FFFF, 0x000000 };
    for (int i=0; i<width; i++)
    {
        int index = (start + i/block) % 8;
        pLine[0] = (colors[index] >> 16) & 0xFF;
        pLine[1] = (colors[index] >>  8) & 0xFF;
        pLine[2] = (colors[index] >>  0) & 0xFF;
        pLine += pixel;
    }
}
...
void YUV_Rgb_CreateBuffer(char* pRgb,
                          ...
                          const int block)
{
    int nLineSize = width*pixel;
    char* pLine;
    int vCount = (height+block-1)/block;
    int bCount = -1;

    pLine  = (char*)malloc(nLineSize);
    for (int j=0; j<height; j++)
    {
        if ((j/block) > bCount)
        {
            bCount ++;
            YUV_Rgb_FillLine(pLine, bCount, width, pixel, block);
        }
        memcpy(pRgb, pLine, width*pixel);
        pRgb += nLineSize;
    }
}
